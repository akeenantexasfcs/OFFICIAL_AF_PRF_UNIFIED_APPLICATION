...
def generate_unified_optimizer_report_docx(
    ...
    # =========================================================================
    # SECTION 6a-2: CUMULATIVE WEALTH INDEX
    # =========================================================================
    doc.add_paragraph()
    _add_heading_paragraph(doc, "Cumulative Wealth Index (Compound Reinvestment)", Pt(14), bold=True,
                           color=_CLR_SLATE_BLUE, space_before=Pt(4))

    cap_wi = doc.add_paragraph()
    cap_wi_run = cap_wi.add_run(
        "Growth of $1 invested in the portfolio strategy, compounding net returns annually. "
        "Net return = indemnity minus producer premium per acre. "
        "A rising line indicates the strategy accumulates wealth over time; "
        "a declining line indicates premium drag exceeds indemnity recovery."
    )
    cap_wi_run.italic = True
    cap_wi_run.font.size = Pt(9)
    cap_wi_run.font.color.rgb = _CLR_SLATE_BLUE

    try:
        # Compound reinvestment: growth of $1
        wealth_index = np.ones(len(all_years) + 1)
        for i, net_ret in enumerate(joint_portfolio):
            annual_return_pct = net_ret / joint_cost if joint_cost > 0 else 0.0
            wealth_index[i + 1] = wealth_index[i] * (1 + annual_return_pct)

        # Plot years with a "Start" point prepended
        plot_x = np.concatenate([[all_years[0] - 1], all_years])

        fig_wi, ax_wi = plt.subplots(figsize=(14, 5), dpi=150)
        fig_wi.patch.set_facecolor(_WARM_CREAM)
        ax_wi.set_facecolor(_WARM_CREAM)

        ax_wi.plot(plot_x, wealth_index, color=_HEX_GREEN, linewidth=2.0)

        # Break-even line
        ax_wi.axhline(y=1.0, color='black', linewidth=0.8, linestyle='--', alpha=0.7)
        ax_wi.annotate('Break-even ($1.00)', xy=(plot_x[-1], 1.0),
                        fontsize=8, color='black', ha='right',
                        xytext=(0, -12), textcoords='offset points')

        # Ending value annotation
        ending_val = wealth_index[-1]
        ax_wi.annotate(f'Ending: ${ending_val:,.2f}', xy=(plot_x[-1], ending_val),
                        fontsize=9, fontweight='bold', color=_HEX_GREEN, ha='right',
                        xytext=(-5, 10), textcoords='offset points')

        ax_wi.set_xlabel('Year', fontsize=10)
        ax_wi.set_ylabel('Portfolio Value ($)', fontsize=10)
        ax_wi.set_title('Cumulative Wealth Index \u2014 Growth of $1 Invested', fontsize=13, fontweight='bold')

        ax_wi.yaxis.set_major_formatter(mticker.FuncFormatter(lambda x, p: f'${x:,.2f}'))

        wi_tick_step = max(1, len(plot_x) // 20)
        ax_wi.set_xticks(plot_x[::wi_tick_step])
        ax_wi.set_xticklabels(plot_x[::wi_tick_step].astype(int), rotation=45, ha='right', fontsize=7)

        ax_wi.spines['top'].set_visible(False)
        ax_wi.spines['right'].set_visible(False)
        ax_wi.grid(axis='y', alpha=0.3)

        plt.tight_layout()
        _add_chart_to_doc(doc, fig_wi)
    except Exception:
        plt.close('all')
        doc.add_paragraph('(Cumulative Wealth Index chart could not be generated.)')
